...
class ESKF:
    ...
    def predict_nominal(
        ...
        q_r = np.append(arr1, np.sin(np.linalg.norm(Kappa)/2) * Kappa.T / np.linalg.norm(Kappa))
        quaternion_prediction = quaternion_product(quaternion, q_r)

        # Normalize quaternion
        quaternion_prediction = quaternion_prediction / np.linalg.norm(quaternion_prediction)

        acceleration_bias_prediction = acceleration_bias*(1 - Ts*self.p_acc) # Vet ikke om dette er riktig
        gyroscope_bias_prediction = gyroscope_bias*(1 - Ts*self.p_gyro) # Samme
    ...
    def predict_covariance(
        ...
        P_predicted = Ad @ P @ Ad.T + GQGd # Riktig?
    ...
    def inject(
        ...
        x_injected[ATT_IDX] = quaternion_product(x_injected[ATT_IDX], np.array([1, *delta_x[ERR_ATT_IDX]/2]))
        x_injected[ATT_IDX] = x_injected[ATT_IDX] / np.linalg.norm(x_injected[ATT_IDX]) # Normalize

        # Covariance
        S = cross_product_matrix(0.5*delta_x[ERR_ATT_IDX]) #Theta_hat 4-dim quaternion?
        G_injected = la.block_diag(np.eye(6), np.eye(3) - S, np.eye(6))  # TODO: Compensate for injection in the covariances
    ...
    def innovation_GNSS_position(
        ...
        H = np.concatenate((np.eye(3), np.zeros((3,12))), axis=1) # Riktig dim?
