...
def quaternion_product(ql: np.ndarray, qr: np.ndarray) -> np.ndarray:
    ...
    S_left = utils.cross_product_matrix(epsilon_left)
    
    quaternion = np.zeros((4,)) 
    quaternion[0] = eta_left*q_right[0] - epsilon_left.T @ q_right[1:]
    quaternion[1:] = (q_right[0]*epsilon_left).reshape(3) + (eta_left*q_right[1:]).reshape(3) + (S_left@q_right[1:]).reshape(3)
...
def quaternion_to_euler(quaternion: np.ndarray) -> np.ndarray:
    ...
    q_squared = quaternion ** 2

    phi = np.arctan2(2*(quaternion[3]*quaternion[2] + quaternion[0]*quaternion[1]), q_squared[0] - q_squared[1] - q_squared[2] + q_squared[3])  # TODO: Convert from quaternion to euler angles
    theta = np.arcsin(2*(quaternion[0]*quaternion[2] - quaternion[1]*quaternion[3]))  # TODO: Convert from quaternion to euler angles
    psi = np.arctan2(2*(quaternion[1]*quaternion[2] + quaternion[0]*quaternion[3]), q_squared[0] + q_squared[1] - q_squared[2] - q_squared[3])  # TODO: Convert from quaternion to euler angles
...
#print(quaternion_product(np.array([4, 5, 3, 2]), np.array([10, 2, 9, 3])))
# a = np.array([4,5,3,2])
# a = a/np.linalg.norm(a)
# print(a)

# print(quaternion_to_rotation_matrix(a))
# print(quaternion_to_euler(a))
